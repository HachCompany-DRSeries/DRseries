...
#define		MT9V032_CHIP_CONTROL_SNAPSHOT_MODE	(1 << 4)
...
//	gpiod_set_value_cansleep(mt9v032->enable_gpio, 1);
	...
	/* wait 10ms for the oscillator to stabilize */
	msleep(10);
	...
	ret = regmap_write(map, MT9V032_CHIP_CONTROL,
			   MT9V032_CHIP_CONTROL_MASTER_MODE | MT9V032_CHIP_CONTROL_SNAPSHOT_MODE | MT9V032_CHIP_CONTROL_SEQUENTIAL);
	if (ret < 0)
		goto err;

	unsigned int readval;
	ret = regmap_read(map, MT9V032_CHIP_CONTROL, &readval);
	if (ret < 0)
		goto err;
	//printk("MT9V032_CHIP_CONTROL %x line = %d\n", readval, __LINE__);
...
{
//	gpiod_set_value_cansleep(mt9v032->enable_gpio, 0);
	clk_disable_unprepare(mt9v032->clk);
}
...
static int mt9v032_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	if (!enable)
	{	
		if (is_mt9v022_rev3(mt9v032->version->version) ||
		    is_mt9v024(mt9v032->version->version)) {
			/*
			 * Required settings for snapshot mode: set bit 9
			 * (RST enable) and bit 2 (CR enable) in reg. 0x20
			 * See TechNote TN0960 or TN-09-225.
			 */
			if (regmap_update_bits(map, MT9V032_REG32, 0x204, 0x204))
				return -EIO;
		}
	...
	if (is_mt9v022_rev3(mt9v032->version->version) ||
		...
		/*
			...
			*/
		if (regmap_update_bits(map, MT9V032_REG32, 0x204, 0))
			return -EIO;
	}
...
#define V4L2_CID_SINGLESHOT		    (V4L2_CID_USER_BASE | 0x1008)
#define V4L2_CID_RW_REGISTER		(V4L2_CID_USER_BASE | 0x1009)
#define V4L2_CID_REGISTER_ADR		(V4L2_CID_USER_BASE | 0x100A)
#define V4L2_CID_CLOCK_ENABLE		(V4L2_CID_USER_BASE | 0x100B)
static u8 register_address;
 ...
 */
	case V4L2_CID_REGISTER_ADR:
		register_address = ctrl->val & 0xff;
		return 0;
 ...
 */
	case V4L2_CID_RW_REGISTER:
		if(register_address == 0 || register_address == 0xff)
		{
			// do not write into readonly registers
			return 0;
		}
		reg_data = regmap_write(map, register_address, ctrl->val);
/*
        pr_err( "s_ctrl1 %x\n", register_address);
        pr_err( "s_ctrl2 %x\n", ctrl->val);
*/
...
*/
	...
	{
        unsigned int val = 0;
        if(regmap_read(map, MT9V032_CHIP_CONTROL, &val) >= 0)
        {
		    if (ctrl->val)
		    {
                val |= 0x10;
            ...
            reg_data = regmap_write(map, MT9V032_CHIP_CONTROL, val);	  
		    if (reg_data < 0)
			    return -EIO;
			unsigned int readval;
			int ret;
...
static int mt9v032_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
    struct mt9v032 *mt9v032 =
			container_of(ctrl->handler, struct mt9v032, ctrls);
	struct regmap *map = mt9v032->regmap;
   /* struct v4l2_ctrl *gain = mt9v022->gain;
    ...
    switch (ctrl->id) {
	case V4L2_CID_EXPOSURE_AUTO:
		retval = regmap_read(map, MT9V032_AEC_AGC_ENABLE, &data);
		if (retval < 0)
		    return -EIO;
		ctrl->val = !!(data & 0x1);
		return 0;
	case V4L2_CID_AUTOGAIN:
		...
		return 0;
	/*case V4L2_CID_GAIN:
		retval = regmap_read(map, MT9V032_ANALOG_GAIN, &data);
		if (retval < 0)
			return -EIO;
		...
		return 0;
	case V4L2_CID_EXPOSURE:
		retval = regmap_read(map, MT9V032_TOTAL_SHUTTER_WIDTH, &data);
		if (retval < 0)
			return -EIO;
		...
		return 0;*/
	case V4L2_CID_HBLANK:
		retval = regmap_read(map, MT9V032_HORIZONTAL_BLANKING, &data);
		if (retval < 0)
			return -EIO;
		ctrl->val = data;
		return 0;
	case V4L2_CID_VBLANK:
		retval = regmap_read(map, MT9V032_VERTICAL_BLANKING, &data);
		if (retval < 0)
			return -EIO;
		ctrl->val = data;
		return 0;
...
*/
	case V4L2_CID_SINGLESHOT:
		retval = regmap_read(map, MT9V032_CHIP_CONTROL, &data);
		if (retval < 0)
			return -EIO;
		ctrl->val = !!(data & 0x10);
		return 0;
...
*/
	case V4L2_CID_RW_REGISTER:
		retval = regmap_read(map, register_address, &ctrl->val);
        if (retval < 0)
			return -EIO;
        return 0;

      case V4L2_CID_CLOCK_ENABLE:
		retval = 0;//gpiod_get_value_cansleep(mt9v032->enable_gpio);
		return 0;
      }
      return -EINVAL;
}
...
static const struct v4l2_ctrl_ops mt9v032_ctrl_ops = {
    ...
    .g_volatile_ctrl = mt9v032_g_volatile_ctrl,
};
...
static const struct v4l2_ctrl_config mt9v022_additional_ctrls[] = {
    {
		.ops		= &mt9v032_ctrl_ops,
	    ...
	    .ops		= &mt9v032_ctrl_ops,
	    .id			= V4L2_CID_CLOCK_ENABLE,
	    .type		= V4L2_CTRL_TYPE_BOOLEAN,
	    .name		= "Clock enable on/off",
	    .min		= 0,
	    .max		= 1,
	    .step		= 1,
	    .def		= 1,
...
//	mt9v032->enable_gpio = devm_gpiod_get_optional(&client->dev, "enable",
//					      GPIOD_OUT_HIGH);
//	if (IS_ERR(mt9v032->enable_gpio))
//		return PTR_ERR(mt9v032->enable_gpio);
	...
	v4l2_ctrl_handler_init(&mt9v032->ctrls, 11 +
			       ARRAY_SIZE(mt9v032_aegc_controls) + ARRAY_SIZE(mt9v022_additional_ctrls));
    ...
    for (i = 0; i < ARRAY_SIZE(mt9v022_additional_ctrls); ++i)
		v4l2_ctrl_new_custom(&mt9v032->ctrls, &mt9v022_additional_ctrls[i],
				     NULL);

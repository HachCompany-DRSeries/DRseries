			    ...
			    u32 height, u32 stride, const char *format)
{
	...
	//ret = fdt_setprop_string(fdt, node, "memory-region", "framebuffer");
	//printf("%s:%d\n", __func__, __LINE__);
	//printf("%s:%d fdt = %x node = %x ret = %d\n", __func__, __LINE__, fdt, node, ret);

	snprintf(name, sizeof(name), "framebuffer@%llx", base_address);
	ret = fdt_set_name(fdt, node, name);
	//printf("%s:%d fdt = %x node = %x name = %s ret = %d\n", __func__, __LINE__, fdt, node, name, ret);
	if (ret < 0)
		return ret;

	ret = fdt_setprop_u32(fdt, node, "width", width);
	//printf("%s:%d fdt = %x node = %x width = %d ret = %d\n", __func__, __LINE__, fdt, node, width, ret);
	if (ret < 0)
		return ret;

	ret = fdt_setprop_u32(fdt, node, "height", height);
	//printf("%s:%d fdt = %x node = %x height = %d ret = %d\n", __func__, __LINE__, fdt, node, height, ret);
	if (ret < 0)
		return ret;

	ret = fdt_setprop_u32(fdt, node, "stride", stride);
	//printf("%s:%d fdt = %x node = %x stride = %d ret = %d\n", __func__, __LINE__, fdt, node, stride, ret);
	if (ret < 0)
		return ret;

	ret = fdt_setprop_string(fdt, node, "format", format);
	//printf("%s:%d fdt = %x node = %x format = %s ret = %d\n", __func__, __LINE__, fdt, node, format, ret);
	if (ret < 0)
		return ret;

	ret = fdt_setprop_string(fdt, node, "status", "okay");
	//printf("%s:%d fdt = %x node = %x status = okay ret = %d\n", __func__, __LINE__, fdt, node, ret);
	if (ret < 0)
		return ret;

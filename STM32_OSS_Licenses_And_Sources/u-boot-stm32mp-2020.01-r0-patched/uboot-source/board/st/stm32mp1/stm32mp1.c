...
/* Function to read OTP 60 & 61 where hw_serial_nr and hw_revision are stored permanently. 
   Converts the values to strings and saves to uboot env.
*/
int get_hwserial_and_rev(void)
{
	int ret;
	struct udevice *dev;
	u32 hwserial, hwrev;
	static char hwserial_str[16], hwrev_str[16];
	...
	if (ret){
		printf("Failed to get BSEC device: %d\n", ret);
		return ret;
	}

	// Read from OTPs and store to hwserial & hwrev
	ret = misc_read(dev, STM32_BSEC_OTP(60),
			&hwserial, sizeof(hwserial));
			
	if (ret < 0) {
		printf("OTP 60 read error \n");
		return ret;
	}

	ret = misc_read(dev, STM32_BSEC_OTP(61),
			&hwrev, sizeof(hwrev));
			
	if (ret < 0) {
		printf("OTP 61 read error \n");
		return ret;
	}

	// Convert to strings and save to env if they have value other than 0
	snprintf(hwserial_str, sizeof(hwserial_str), "%x", hwserial);
	snprintf(hwrev_str, sizeof(hwrev_str), "r%x", hwrev);
	
	if (strcmp(hwserial_str,"0") != 0){
		printf("hw_serial_nr: %s \n",hwserial_str);
		env_set("hw_serial_nr", hwserial_str);
	}
	if (strcmp(hwrev_str,"r0") != 0){
		printf("hw_revision: %s \n",hwrev_str);
		env_set("hw_revision", hwrev_str);
	}
	
	return 0;
}

/* Function to supress console output when security stick is not mounted */
int securitycheck_usb(void) {
    int ret;
    
    // Start USB
    ret = run_command("usb start", 0);
    if (ret) { 
		printf("USB tree not available\n");
		return -1;
    }

    // Check if the file exists on USB drive 0, partition 1
    ret = run_command("fatload usb 0:1 ${loadaddr} root_upd_viper.sh 1", 0);
    if (ret == 0) {  // fatload returns 0 on success (file exists)
...
/*This function has no use if those keys aren't present*/
//	board_key_check();
...
int ft_board_setup(void *blob, bd_t *bd)
{
	...
	//printf("%s\n",__func__);
	//if (CONFIG_IS_ENABLED(LCD_DT_SIMPLEFB))
		lcd_dt_simplefb_enable_and_mem_rsv(blob);

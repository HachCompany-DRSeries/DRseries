 ...
 * @param  dec
 * @param  reg
 * @return fixedErrSum | DmtxFail
 */
extern int
CheckAnzCorrections(DmtxDecode *dec, DmtxRegion *reg)
{
   int i, j;
   int interleavedBlocks;
   int blockErrorWords;
   ...
   interleavedBlocks   = dmtxGetSymbolAttribute(DmtxSymAttribInterleavedBlocks, reg->sizeIdx);
   blockErrorWords     = dmtxGetSymbolAttribute(DmtxSymAttribBlockErrorWords, reg->sizeIdx);
   blockMaxCorrectable = dmtxGetSymbolAttribute(DmtxSymAttribBlockMaxCorrectable, reg->sizeIdx);

   fixedErrSum = 0;
   for(i = 0; i < interleavedBlocks; i++) {

      blockTotalWords = blockErrorWords + dmtxGetBlockDataSize(reg->sizeIdx, i);
      ...
      for(j = 0; j < blockTotalWords; j++)
         data[j] = msg->code[j*interleavedBlocks+i];

      fixedErrSum += decode_rs_char(rs, data, NULL, 0, -1);;

      for(j = 0; j < blockTotalWords; j++)
         msg->code[j*interleavedBlocks+i] = data[j];

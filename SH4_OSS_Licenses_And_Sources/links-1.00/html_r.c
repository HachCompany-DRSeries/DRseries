...
{
#if 0 
  // split in Print_text() because this split doesn't work properly:
  // -> lines are diplayed twice!!!
  int i;
  /*if (!p->data) goto r;*/
...
void *html_special(struct part *p, int c, ...)
{
	...
	switch (c) {
		case SP_TAG:
			...
			va_end(l);
			html_tag(p->data, t, (X(p->cx) == -1 ? 0 : X(p->cx)), Y(p->cy));
			break;
		case SP_CONTROL:
...
void push_base_format(unsigned char *url, struct document_options *opt)
{
	...
	if (html_stack.next != &html_stack) {
	  kill_html_stack_item(html_stack.next); // MS: added 9.12.15

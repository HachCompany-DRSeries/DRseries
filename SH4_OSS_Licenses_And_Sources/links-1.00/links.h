...
#define LINKS_COPYRIGHT_8859_1 "(C) 1999 - 2008 Mikul�s Patocka"
#define LINKS_COPYRIGHT_8859_2 "(C) 1999 - 2008 Mikul� Pato�ka"
...
{
	unsigned char *m;
	m = (unsigned char *) debug_mem_alloc((unsigned char *)f, (int) l, (int)len + 1);
	memcpy(m, src, len);
	m[len] = 0;
	return m;
}
...
{
	return src ? debug_memacpy((unsigned char *)f, (int) l, (unsigned char *) src, src != DUMMY ? strlen((const char *)src) : 0) : NULL;
}
...
{
	unsigned char *p;
	size_t l1 = strlen((const char *) *s), l2 = strlen((const char *) a);
	if (((l1 | l2) | (l1 + l2 + 1)) > MAXINT) overalloc();
	p = (unsigned char *) mem_realloc(*s, l1 + l2 + 1);
	strcat((char *) p, (const char *) a);
	*s = p;
}
...
{
	unsigned char *s = (unsigned char *) debug_mem_alloc((unsigned char *) file, (int) line, ALLOC_GR);
	*s = 0;
	return s;
}
...
{
	unsigned char *p;
	size_t ll = (size_t) strlen((const char *) a);
	if (ll > MAXINT || *l + ll > MAXINT) overalloc();
	p = *s;
	if (((size_t)*l & ~(ALLOC_GR - 1)) != ((*l + ll) & ~(ALLOC_GR - 1))) {
		p = *s = (unsigned char *) mem_realloc(*s, (*l + ll + ALLOC_GR) & ~(ALLOC_GR - 1));
	}
	strcpy((char *) p + *l,(const char *) a); *l += ll;
}
...
static inline void add_chr_to_str(unsigned char **s, int *l, unsigned char a)
{
	...
	if ((*l & (ALLOC_GR - 1)) == ALLOC_GR - 1) {
		p = *s = (unsigned char *) mem_realloc(*s, (*l + 1 + ALLOC_GR) & ~(ALLOC_GR - 1));
	}
	*(p + *l) = a; *(p + ++(*l)) = 0;
}
...
{
	unsigned char a[13];
	if (n && n / (1024 * 1024) * (1024 * 1024) == n) snzprint((unsigned char *) a, 12, n / (1024 * 1024)), strcat((char *) a, "M");
	else if (n && n / 1024 * 1024 == n) snzprint((unsigned char *) a, 12, n / 1024), strcat((char *) a, "k");
...
/*	size_t to_copy;
	...
	if (strlen((const char *) src) >= dst_size) dst[dst_size - 1] = 0;
	return dst;
}
...
#endif
#define free_list(l) {struct xlist_head *a__;do_not_optimize_here(&l);foreach(a__,l) do_not_optimize_here(a__);foreachback(a__,l) do_not_optimize_here(a__); while ((l).next != &(l)) {a__=(l).next; del_from_list(a__); mem_free(a__); a__=NULL;} do_not_optimize_here(&l);}
...
//void set_line(struct terminal *, int, int, int, chr *, void * f_data);
void set_line(void * f_data);
...
struct tag {
	...
	int num; // Number of the element described by the tag
	int elem; // Type of tag
	unsigned char name[20]; // Value of the tag
};
...
extern unsigned char app_name[];
extern unsigned char app_version[];

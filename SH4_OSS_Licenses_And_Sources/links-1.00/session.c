...
//extern struct session *se;
extern struct window *wi;
struct list_head downloads = {&downloads, &downloads};
extern int webUpdateVersion;
...
int are_there_downloads()
{
  ...
  foreach(down, downloads) if (!down->prog && down->stat.state == S_TRANS) d = 1;
  return d;
}
...
unsigned char *get_err_msg(int state)
{
  ...
  if (state <= S_OK || state >= S_WAIT) 
  {
    ...
    for (i = 0; msg_dsc[i].msg; i++)
      ...
      {
        if (strlen(msg_dsc[i].msg))
          printf("ERR: %s: %s\n",__FUNCTION__,msg_dsc[i].msg);
        return msg_dsc[i].msg;
      }
  unk:
    printf("ERR: %s: %s\n",__FUNCTION__,TEXT(T_UNKNOWN_ERROR));
    return TEXT(T_UNKNOWN_ERROR);
  }
  if (!(e = strerror(-state)) || !*e) goto unk;
  foreach(s, strerror_buf) if (!strcmp(s->msg, e))
  {
    if (strlen(s->msg))
      printf("ERR: %s: %s\n",__FUNCTION__,s->msg);
    return s->msg;
...
unsigned char *get_stat_msg(struct status *stat, struct terminal *term)
{
  ...
  // printf("LINKS: %s\n",stracpy(_(get_err_msg(stat->state), term)));
...
void abort_download(struct download *down)
{
  ...
  mem_free(down);
  if (cancel == 0) {
    // printf("abort_download : pthread_exit\n");
    pthread_exit(NULL);
  }
}
...
int create_download_file(struct terminal *term, unsigned char *fi, int safe)
{
  ...
  if (h == -1) {
    printf("ERR: %s: open %s failed (%d)\n",__FUNCTION__,file,errno);
  ...
  if (strlen(file) >= MAX_STR_LEN) 
  {
    memcpy(download_dir, file, MAX_STR_LEN - 1), download_dir[MAX_STR_LEN - 1] = 0;
  }
  else
  {
    strcpy(download_dir, file);
  }
...
void continue_download(struct session *ses, unsigned char *file)
{
  ...
  if (webUpdate == 1) 
    file = "/tmp/update.tar.gz";
  else 
  {
    if (strlen(file)+10 > sizeof(gFile))
      return; // ERR
    strcpy(gFile,"~/tmp/");
    strcat(gFile,file);
    file = gFile;
  ...
  if ((h = create_download_file(ses->term, file, !!ses->tq_prog)) < 0) {
    if (h == -2 && ses->tq_prog) {
      ...
      msg_box(ses->term, NULL, TEXT(T_DOWNLOAD_ERROR), AL_CENTER | AL_EXTD_TEXT,
              TEXT(T_COULD_NOT_CREATE_TEMPORARY_FILE), NULL, NULL, 1, 
...
void ses_goto(struct session *ses, unsigned char *url, unsigned char *target, int pri, int cache, int wtd, unsigned char *pos, void (*fn)(struct status *, struct session *), int redir)
{
  ...
  if (!strchr(url, POST_CHAR) || (cache == NC_ALWAYS_CACHE && !find_in_cache(url, &e) && !e->incomplete)) {
...
void doc_end_load(struct status *stat, struct session *ses)
{
  if (stat->state < 0) 
  {
    if (ses->display_timer != -1) 
      kill_timer(ses->display_timer), ses->display_timer = -1;
    html_interpret(ses);
    draw_formatted(ses);
    load_frames(ses, ses->screen);
    process_file_requests(ses);
    // printf("doc_end_load : pthread_exit\n");
    pthread_exit(NULL);
...
void destroy_session(struct session *ses)
{
  ...
  if (ses->screen) 
  {
    detach_formatted(ses->screen), mem_free(ses->screen), ses->screen=NULL;
  }

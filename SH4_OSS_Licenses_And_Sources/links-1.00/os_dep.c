...
#if defined(UNIX) || defined(BEOS) || defined(RISCOS) || defined(WIN32) || defined(SPAD)
...
int get_terminal_size(int fd, int *x, int *y)
{
	...
	if (ioctl(1, TIOCGWINSZ, &ws) != -1) {
		if (!(*x = ws.ws_col) && !(*x = get_e("COLUMNS"))) *x = 1000;
...
#if defined(UNIX) || defined(BEOS) || defined(RISCOS) || defined(SPAD)
...
#if defined(UNIX) || defined(OS2) || defined(RISCOS) || defined(SPAD) || defined(WIN32)
...
#ifndef HAVE_STRTOUL
// REMOVE: overwriting 'strtoul' with 'strtol',
// otherwise: When HWC/camera adjust is called from customer app
//            (e.g. dr3900) on Linux kernel 4.1, this leads to error 
//            in function setup_uio_map() in veu_colorspace.c 
//            returning an incorrect ump-addr. The result is that
//            the camera flash stays dark (consecutive fault).

...
#include "links.h"
#endif
#ifndef DEF_MAINLINKS
#define DEF_MAINLINKS
#include "mainlinks.h"
#endif
#include <fcntl.h>
#include <string.h>
...
//#include "peg.hpp"
//#include "SH_CO_Edit.hpp"

struct f_data *fdat;

int webUpdateVersion = 0, webUpdateAuto = 0, webUpdate = 0, webPdf = 0;
int retval = RET_OK;
int defout;
int defin;
int deferr;
...
int attach_terminal(int in, int out, int ctl, void *info, int len)
{
  ...
  if (!info)
  {
    printf("ERR: attach_terminal w info=NULL\n");
    return -1;
  }
  // printf("%s:...\n",__FUNCTION__);
...
void init()
{
  ...
  if (!dmp) {
    if (!((info = create_session_info(base_session, u, &len)) && 
          attach_terminal(get_input_handle() , get_output_handle(), get_ctl_handle(), info, len) != -1)) 
    {
      retval = RET_FATAL;
      terminate = 1;
    }
...
void terminate_all_subsystems()
{
  ...
  abort_all_downloads();
  check_bottom_halves();
  // destroy_all_terminals(); // MS: moved to UI because browser needs fdat
  check_bottom_halves();
  free_all_itrms();
...
void cleanup_handler(void *arg)
{
  ...
  free(argv[1]);
  if (fg_poll_timer != -1) 
    kill_timer(fg_poll_timer), fg_poll_timer = -1;
  if (signal_pipe[0] || signal_pipe[1])
  {
    close(signal_pipe[0]);
    signal_pipe[0] = 0;
    close(signal_pipe[1]);
  ...
  if (terminal_pipe[0] || terminal_pipe[1])
  {
    close(terminal_pipe[0]);
    terminal_pipe[0] = 0;
    close(terminal_pipe[1]);
...
void *mainlinks(void *http_req)
{
  int argc = 2;
  printf("mainlinks: address=%s\n", ((t_http_req*)http_req)->address);
  terminate = 0;

  pthread_cleanup_push(cleanup_handler, NULL);
  
  argv[1] =(unsigned char *) malloc( strlen(((t_http_req*)http_req)->address) + 1 );
  strcpy((char*)argv[1],((t_http_req*)http_req)->address);
  strcpy(app_name,((t_http_req*)http_req)->app_name);
  strcpy(app_version,((t_http_req*)http_req)->app_version);

				    ...
				    struct fb_info *info)
{
	...
	if (!ovl->format->yuv) {
		y_offset = (var->yoffset * ovl->xres_virtual) + (var->xoffset * (ovl->format->bpp / 8));
		c_offset = 0;
	} else {
			      ...
			      struct fb_info *info)
{
	...
	if (!ch->format->yuv) {
		y_offset = (var->yoffset * info->fix.line_length) + (var->xoffset * (info->var.bits_per_pixel / 8)); 
		c_offset = 0;
	} else {
	...
	lcdc_write_chan_mirror(ch, LDSA2R, y_offset);
	...
	case FBIO_WAITFORVSYNC:
		if (priv->pan_in_progress) {
			/* don't wait for panning to be completed using
			   sh_mobile_lcdc_wait_for_vsync but use
			       ...
			       unsigned int num_modes)
{
	...
	info->fix.smem_start = ch->cfg->bootlogo.smem_start;
...
sh_mobile_lcdc_channel_init(struct sh_mobile_lcdc_chan *ch)
{
	...
	/* Allocate frame buffer memory. */
	ch->fb_size = (max_size * format->bpp / 8) * 3;

	info = ch->info;

	if (cfg->bootlogo.size &&
	    cfg->bootlogo.size != ch->fb_size) {
		dev_err(dev, "bootlogo framebuffer size mismatch %u %u\n", cfg->bootlogo.size, ch->fb_size);
		cfg->bootlogo.size = 0;
	}

	if (!cfg->bootlogo.size) {
		buf = dma_alloc_coherent(dev, ch->fb_size,
					  &ch->dma_handle, GFP_ATOMIC);
		if (!buf) {
			dev_err(dev, "unable to allocate buffer\n");
		}
	} else {
		/* framebuffer has already been allocated by
		  * setup.c/bootloader */
		buf = cfg->bootlogo.screen_base;
		ch->dma_handle =cfg->bootlogo.smem_start;
	}

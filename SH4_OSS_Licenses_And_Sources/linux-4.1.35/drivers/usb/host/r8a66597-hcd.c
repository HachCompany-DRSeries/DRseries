...
/* set it to HSE to enable High-Speed */
static unsigned short speed_flag = HSE;
...
static void r8a66597_enable_port(struct r8a66597 *r8a66597, int port)
{
	...
	r8a66597_bset(r8a66597, speed_flag, get_syscfg_reg(port));
...
static void r8a66597_disable_port(struct r8a66597 *r8a66597, int port)
{
	...
	r8a66597_bclr(r8a66597, speed_flag, get_syscfg_reg(port));
}
...
static void r8a66597_oc_timer(unsigned long _r8a66597)
{
	struct r8a66597 *r8a66597 = (struct r8a66597 *)_r8a66597;
	unsigned long flags;
	int oc_status = gpio_get_value(r8a66597->pdata->oc.gpio)==r8a66597->pdata->oc.active_level; /* GPIOLIB is slow, do it outside the spinlock */

	spin_lock_irqsave(&r8a66597->lock, flags);

	if (oc_status && !r8a66597->oc_status) {
		int port;
		/* Let the stack clear the overcurrent event before we signal it again */
		r8a66597->oc_status = oc_status;
		for (port = 0; port < r8a66597->max_root_hub; port++) {
			struct r8a66597_root_hub *rh = &r8a66597->root_hub[port];
			/* disable power on overcurrent event */
	...
	/* poll again */
	mod_timer(&r8a66597->oc_timer,
		  jiffies+msecs_to_jiffies(R8A66597_OC_POLL_TIME));
}
...
static int r8a66597_start(struct usb_hcd *hcd)
{
	...
	/* Timer is not running, so locking is not needed */
	/* We need to enable power first to be able to detect an overcurrent,
	   so we can set the status to no-overcurrent */
	r8a66597->oc_status = 0;

	if (r8a66597->pdata->oc.poll_for_oc)
		mod_timer(&r8a66597->oc_timer,
			  jiffies+msecs_to_jiffies(R8A66597_OC_POLL_TIME));
...
static void r8a66597_stop(struct usb_hcd *hcd)
{
	...
	if (r8a66597->pdata->oc.poll_for_oc)
		del_timer_sync(&r8a66597->oc_timer);
...
static int r8a66597_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	*buf = 0;	/* initialize (no change) */
	...
	if (r8a66597->oc_status)
		*buf |= 0x1; /* signal C_OVER_CURRENT to hub.c */
				    ...
				    struct usb_hub_descriptor *desc)
{
	...
	desc->wHubCharacteristics = cpu_to_le16(characteristics | 0x1);
				...
				u16 wIndex, char *buf, u16 wLength)
{
	...
	case GetHubStatus:
	    	{
			struct usb_hub_status *hub_status = (struct usb_hub_status *)buf;
			/* When we detect overcurrent, we signal it to hub.c
			   and disable the power to the device. Without power,
			   we no longer detect overcurrent. So, we would have
			   another overcurrent change to the no-overcurrent
			   state.  To avoid this, we signal only a change to
			   the overcurrent case and let hub.c clear the
			   overcurrent flag. */
			int change = r8a66597->oc_status ? HUB_CHANGE_OVERCURRENT : 0;
			int status = r8a66597->oc_status ? HUB_STATUS_OVERCURRENT : 0;
			hub_status->wHubStatus = cpu_to_le16(status);
			hub_status->wHubChange = cpu_to_le16(change);
			break;
		}
	case GetPortStatus:
		if (wIndex > r8a66597->max_root_hub)
...
static int r8a66597_remove(struct platform_device *pdev)
{
	...
	if (r8a66597->pdata->oc.poll_for_oc) {
		del_timer_sync(&r8a66597->oc_timer);
		gpio_free(r8a66597->pdata->oc.gpio);
	}
...
static int r8a66597_probe(struct platform_device *pdev)
{
	...
	r8a66597->irq_sense_low = irq_trigger == IRQF_TRIGGER_LOW;
        ...
        speed_flag = r8a66597->pdata->disable_highspeed ? 0 : HSE;
        printk(KERN_INFO "USB High-Speed is %s\n",
               r8a66597->pdata->disable_highspeed ? "disabled" : "enabled");
	...
	if (r8a66597->pdata->oc.poll_for_oc) {
		ret = gpio_request(r8a66597->pdata->oc.gpio,
				   "r8a66597-hcd:oc");
		if (ret<0) {
			dev_err(&pdev->dev, "Failed to add GPIO\n");
			goto clean_gpio;
		}

		gpio_direction_input(r8a66597->pdata->oc.gpio);
		setup_timer(&r8a66597->oc_timer, r8a66597_oc_timer,
				(unsigned long)r8a66597);
	}

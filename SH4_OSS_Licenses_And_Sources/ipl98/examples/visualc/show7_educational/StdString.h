...
#ifndef SS_USE_FACET
	#if defined(__SGI_STL) || defined(__SGI_STL_PORT)
...
// -----------------------------------------------------------------------------
#define StrSizeType std::string::size_type	// just for shorthand, really
...
inline void	ssasn(std::string& sDst, const std::wstring& sSrc)
{
	...
	sDst.resize(0);
	sDst.resize(nLen);
	StdCodeCvt(const_cast<std::string::pointer>(sDst.data()),
			   sSrc.c_str(),
			   nLen);
#else
	SSCVT;
...
inline void	ssasn(std::string& sDst, PCWSTR pW)
{
	...
	sDst.resize(0);
	sDst.resize(nLen);
	StdCodeCvt(const_cast<std::string::pointer>(sDst.data()), pW, nLen);
#else
	SSCVT;
...
inline void	ssasn(std::wstring& sDst, const std::string& sSrc)
{
	...
	sDst.resize(0);
	sDst.resize(nLen);
	StdCodeCvt(const_cast<std::wstring::pointer>(sDst.data()),
				 sSrc.c_str(),
				 nLen);
#else
	SSCVT;
...
inline void	ssasn(std::wstring& sDst, PCSTR pA)
{
	...
	sDst.resize(0);
	sDst.resize(nLen);
	StdCodeCvt(const_cast<std::wstring::pointer>(sDst.data()), pA, nLen);
#else
	SSCVT;
...
inline void	ssadd(std::string& sDst, const std::wstring& sSrc)
{
	...
	StdCodeCvt(const_cast<std::string::pointer>(sDst.data()+nLen),
			   sSrc.c_str(),
			   nLen);
#else
	SSCVT; 
...
inline void	ssadd(std::string& sDst, PCWSTR pW)
{
	...
	int nLen	= sslen(pW);
	sDst.resize(sDst.size() + nLen);
	StdCodeCvt(const_cast<std::string::pointer>(sDst.data()+nLen), pW, nLen);
#else
	SSCVT;
	if ( NULL != pW )
...
inline void	ssadd(std::wstring& sDst, const std::string& sSrc)
{
	...
	int nLen	= sSrc.size();
	sDst.resize(sDst.size() + nLen);
	StdCodeCvt(const_cast<std::wstring::pointer>(sDst.data()+nLen),
				 sSrc.c_str(),
				 nLen);
#else
	SSCVT;
...
inline void	ssadd(std::wstring& sDst, PCSTR pA)
{
	...
	int nLen	= sslen(pA);
	sDst.resize(sDst.size() + nLen);
	StdCodeCvt(const_cast<std::wstring::pointer>(sDst.data()+nLen), pA, nLen);
#else
	SSCVT;
	if ( NULL != pA )
...
int sscpysame(CT* pDst, const CT* pSrc, int nMax=-1)
{
	...
	std::char_traits<CT>().copy(pDst, pSrc, nChars * sizeof(CT));
	if ( nChars > 0 )
		pDst[nChars]	= '\0';
...
class CStdStr : public std::basic_string<CT>
{
	...
	typedef typename std::basic_string<CT>	MYBASE;	 // my base class (string | wstring)
...
public:
	...
	// constructors
							...
							CStdStr(const std::string& strA);
							CStdStr(const std::wstring& strW);
	...
	// assignment operators
			...
			CStdStr<CT>&	operator=(const std::string& strA);	
			CStdStr<CT>&	operator=(const std::wstring& strW);
	...
	// *** Thanks to Pete The Plumber for catching this one ***
			...
			CStdStr<CT>&	assign(const CStdStr<CT>& str,
								   MYSIZE nStart,
								   MYSIZE nChars);
			CStdStr<CT>&	assign(const std::basic_string<CT>& str);
			CStdStr<CT>&	assign(const std::basic_string<CT>& str,
								   MYSIZE nStart,
								   MYSIZE nChars);
	...
	// CString facade functions -- make us a drop-in replacement for CString (almost).  
			...
			int				ReverseFind(CT ch) const;				// find starting from the back
...
// They also are compiled if you have explicitly turned off refcounting
	...
	template<typename CT>
	inline
	CStdStr<CT>& CStdStr<CT>::assign(const std::basic_string<CT>& str)
	{
		ssasn(*this, str);
		return *this;
	}
...
#else
	inline bool ssisp(char c) { return FALSE != ::isspace((int) c); }
	inline bool ssisp(wchar_t c) { return FALSE != ::iswspace((wint_t) c); }
	inline bool operator() (CT t) { return !ssisp(t); }
#endif
};
...
{
	std::locale loc;
	MYITER it = std::find_if(begin(), end(), NotSpace<CT>(loc));
	if ( begin() != it ) // avoid  assignment when possible
	{
...
#ifndef SS_ANSI
template<typename CT>
void CStdStr<CT>::FormatMessage(PCMYSTR szFormat, ...) throw(std::exception)
{
	va_list argList;
	va_start(argList, szFormat);
	PMYSTR szTemp;
...
template<typename CT>
void CStdStr<CT>::FormatMessage(UINT nFormatId, ...) throw(std::exception)
{
	MYTYPE sFormat;

...
void
fz_run_display_list(fz_display_list *list, fz_device *dev, const fz_matrix *top_ctm, const fz_rect *scissor, fz_cookie *cookie, int iMaxImageRawSize, fz_rect *pBlockedImage )
{
	fz_display_node *node;
	fz_matrix ctm;
	int clipped = 0;
		...
		fz_try(ctx)
		{
			...
			case FZ_CMD_FILL_IMAGE:
				if ((dev->hints & FZ_IGNORE_IMAGE) == 0){
					if( iMaxImageRawSize>0 && (node->item.image->w*node->item.image->h*3)>iMaxImageRawSize )
					{  
					    printf("FITZ, image blocked: w=%i, h=%i, size=%lu KB\n",
					      node->item.image->w, node->item.image->h,
						(node->item.image->w * node->item.image->h * 3)/1024);
					    if( pBlockedImage) *pBlockedImage = node_rect;  
					}
					else{

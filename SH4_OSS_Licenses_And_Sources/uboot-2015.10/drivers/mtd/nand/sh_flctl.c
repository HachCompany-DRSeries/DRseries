...
static int read_ecfiforeg(struct sh_flctl *flctl, u8 *buff, int sector)
{
	int i;
...
static void execmd_read_page_sector(struct mtd_info *mtd, int page_addr)
{
	...
	for (sector = 0; sector < page_sectors; sector++) {
		...
		ret = read_ecfiforeg(flctl,
			&flctl->done_buff[mtd->writesize + 16 * sector], sector);
...
int board_nand_init(struct nand_chip *nand)
{
	...
	struct mtd_info *flctl_mtd;
        flctl = kmalloc(sizeof(struct sh_flctl), GFP_KERNEL);
        if (!flctl) {
                printf("out of memory\n");
                return -1;
        }
        memset(flctl, 0, sizeof(*flctl));
        nand->priv = flctl;

        flctl->reg = (void*) 0xA4530000;

	flctl_mtd = &flctl->mtd;
	flctl_mtd->priv = nand;
#ifdef NAND_USE_HWECC
	flctl->hwecc = 1;
#endif
        nand->ecc.mode    = flctl->hwecc ? NAND_ECC_HW : NAND_ECC_SOFT;

	flctl_register_init(flctl, SH_FLCTL_FLCMNCR_NAND_TIMING | TYPESEL_SET | NANWF_E);

...
#endif
				if (pc->page_list)
					fz_run_display_list(pc->page_list, dev, &ctm, &rect, NULL, 0, NULL);
				if (pc->annot_list)
					fz_run_display_list(pc->annot_list, dev, &ctm, &rect, NULL, 0, NULL);
#ifdef TIME_DISPLAY_LIST
			}
			time = clock() - time;
	...
	fz_try(ctx)
	{
		...
		for (crect = hq ? pc->hq_changed_rects : pc->changed_rects; crect; crect = crect->next)
		{
			...
			if (!fz_is_empty_irect(&abox))
			{
				...
				if (pc->page_list)
					fz_run_display_list(pc->page_list, dev, &ctm, &arect, NULL, 0, NULL);
				if (pc->annot_list)
					fz_run_display_list(pc->annot_list, dev, &ctm, &arect, NULL, 0, NULL);
				fz_free_device(dev);
				dev = NULL;
			}
		}

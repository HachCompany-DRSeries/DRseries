...
/*       break; */
      ...
      case DmtxPack16bppYCbCr:
         ...
         break;
      case DmtxPack16bppRGB565:
         err = dmtxImageSetChannel(img,  0, 5);
         err = dmtxImageSetChannel(img,  5, 6);
...
dmtxImageGetPixelValue(DmtxImage *img, int x, int y, int channel, int *value)
{
 ...
 unsigned char *pixelPtr;
   ...
   if (img->pixelPacking == DmtxPack16bppRGB565) {
           y = img->height-y;
           unsigned short pixel = *(((unsigned short*) img->pxl) + y*img->width+x);
           ...
           if (channel==0)
                   ...
                   *value = (pixel&0x1f)<<3;
                   ...
                   /* green */
                   *value = ((pixel>>5)&0x3f)<<2;
           else
                   /* red */
                   *value = ((pixel>>11)&0x1f)<<3;
           return DmtxPass;
   }
...
/*       assert(img->bitsPerPixel == 1);
      ...
      case 6:
       ...
       assert(img->bitsPerPixel == 16);
         pixelPtr = img->pxl + (offset * (img->bitsPerPixel/8));
         pixelValue = (*pixelPtr << 8) | (*(pixelPtr+1));
         bitShift = img->bitsPerPixel - 6 - img->channelStart[channel];
         mask = 0x3f << bitShift;
         *value = (((pixelValue & mask) >> bitShift) << 2);
         break;
      case 8:

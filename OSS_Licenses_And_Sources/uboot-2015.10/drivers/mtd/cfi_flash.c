				  ...
				  int len)
{
	...
	while ((cnt-- > 0) && (flag == 1)) {
		...
		case FLASH_CFI_8BIT:
			...
			break;
		case FLASH_CFI_16BIT:
			flag = (((flash_read16(dst2) & flash_read16(src)) | 0xffff0000) ==
				(flash_read16(src) | 0xffff0000));
			src += 2, dst2 += 2;
			break;
		case FLASH_CFI_32BIT:
...
void flash_print_info (flash_info_t * info)
{
	...
	for (i = 0; i < info->sector_count; ++i) {
		const char *status = info->protect[i] ? "RO" : "  ";

#ifdef CONFIG_SYS_FLASH_PROTECTION_BLOCK_LOCK_DOWN
		flash_write_cmd(info, 0, 0, FLASH_CMD_READ_ID);
		if (flash_isset (info, i,
				 FLASH_OFFSET_PROTECT,
				 FLASH_STATUS_PROTECT_LOCKED))
			status = "LO";
	...
	switch (info->vendor) {
		...
		case CFI_CMDSET_INTEL_EXTENDED:
			...
			if (prot) {
				...
				flash_write_cmd (info, sector, 0, FLASH_CMD_PROTECT_LOCK_DOWN);
			}
#endif
			break;
		case CFI_CMDSET_AMD_EXTENDED:
		...
		for (i = 0; i < num_erase_regions; i++) {
			...
			for (j = 0; j < erase_region_count; j++) {
				...
				if (sect_cnt >= CONFIG_SYS_MAX_FLASH_SECT) {
                                  printf("ERROR: too many flash sectors (%d>%d)\n", sect_cnt, CONFIG_SYS_MAX_FLASH_SECT);
                                  break;
				}
				info->start[sect_cnt] =
...
{
#if 0  /* comment out here because in board_late_init of HiCO.DIMMSH7723
          we unprotect the complete flash. This needs a lot of time
          for spansion S29GL064S */

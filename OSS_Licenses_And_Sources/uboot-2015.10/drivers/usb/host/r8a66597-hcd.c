			       ...
			       void *buffer, int transfer_len)
{
	...
	int timeout = 10000; /* 10000 * 100us = 1 sec */
	int rcv_len;
				  ...
				  void *buffer, int transfer_len)
{
	...
	/* FIXME: limit transfer size : 64byte or less, emtrion: done so far for some special hubs */
	do {
		...
		if ( flag == 0 ) {
			...
			r8a66597_bclr(r8a66597, SQSET, DCPCTR);
			flag = 0;
		}
		r8a66597_write(r8a66597, BCLR, CFIFOCTR);
		...
		buffer += rcv_len;
/* as long as we didn't get the whole buffer and max packages (limitation by device) are sent */
	} while ( (dev->act_len < transfer_len) /*&& (transfer_len != 64)*/ && (dev->epmaxpacketin[0] == rcv_len) );
	return 0;
}
...
#ifdef CONFIG_USB_HAS_BOARD_INIT
	if (board_usb_init(0,0))
		return -1;

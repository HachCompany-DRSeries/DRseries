...
void draw_doc(struct terminal *t, struct f_data_c *scr, int active)
{
	...
	check_vs(scr);
	vs = scr->vs;
	if (vs->goto_position && !get_cache_entry(vs->url, &ce) && (vy = find_tag(scr->f_data, vs->goto_position)) != -1) {
		if (vs->goto_position_end) mem_free(vs->goto_position_end);
		if (ce->incomplete) {
	...
	if (vs->goto_position_end && !get_cache_entry(vs->url, &ce) && !ce->incomplete && (vy = find_tag(scr->f_data, vs->goto_position_end)) != -1) {
...
//		set_line(t, xp + st - vx, yp + y - vy, en - st, &scr->f_data->data[y].d[st], scr->f_data);
		set_line(scr->f_data);
//		printf("link : %02x\n", (int) &scr->f_data->data[y].d[st]);
//		sleep(2);
	}
	draw_forms(t, scr);
	if (active) draw_current_link(t, scr);
...
#else
void init_vs(struct view_state *vs, unsigned char *url){}
int dump_to_file(struct f_data *fd, int h){return 0;}
void detach_formatted(struct f_data_c *scr){}
void destroy_vs(struct view_state *vs){}
struct f_data_c *current_frame(struct session *ses){return 0;}
void copy_location(struct location *dst, struct location *src){}
unsigned char *print_current_title(struct session *ses){return 0;}
unsigned char *print_current_link(struct session *ses){return 0;}
void draw_formatted(struct session *ses){}
void send_event(struct session *ses, struct event *ev){}
int get_current_state(struct session *ses){return 0;}
void selected_item(struct terminal *term, void *pitem, struct session *ses){}
void destroy_formatted(struct f_data *scr){}
void destroy_fc(struct form_control *fc){}
void sort_links(struct f_data *f){}

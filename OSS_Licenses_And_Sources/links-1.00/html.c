...
// Function to get the number of characters concerned by a tag
int nb_char_tag(unsigned char * tmp_chr) {
  int nb_loop = 1, nb_chr = 0, curs = 0, curs_save;
  // while (tmp_chr[nb_chr] != (unsigned char) '<')
  //  nb_chr++;
  while (nb_loop != 0) {
    while (tmp_chr[curs] != (unsigned char) '<') {
      if (!tmp_chr[curs]) return nb_chr; // MS 05.10.15
      
      // If there is a special character (for example ä = &#xE4;) it must count as one character
      if (tmp_chr[curs] == (unsigned char) '&') {
        curs_save = curs;
        curs++;
        while (tmp_chr[curs] != (unsigned char) ';' && tmp_chr[curs] != (unsigned char) '&'
               && tmp_chr[curs] != (unsigned char) '/' && tmp_chr[curs] != (unsigned char) ' ')
        {
          if (!tmp_chr[curs]) return nb_chr; // MS 05.10.15
          curs++;
        }
        if (tmp_chr[curs] != (unsigned char) ';') // Character ';' not found : it was not a special character
        {
          curs = curs_save;
        }
        nb_chr++;
    ...
    while(tmp_chr[curs] != (unsigned char) '>'){
      if (!tmp_chr[curs]) return nb_chr; // MS 25.03.11
      curs++;
    }
    curs++;
  }
...
void parse_html(unsigned char *html, unsigned char *eof, int (*put_chars)(void *, unsigned char *, int), void (*line_break)(void *), void *(*special)(void *, int, ...), void *f, unsigned char *head)
{
  ...
  /* (this function is called various times until page is complete) */
  /******************************************************************/
  if ((lt = strstr(html,"</html>"))){
    *(lt+7) = 0; // force EOS after </html>
  }
  else
    return; // download not ready
  
  /* Handle Error 404 */
  /********************/
  if (strstr(html, "Error 404")){
    strcpy(html,"<html><body> Page not found </body></html>");
    eof = html+strlen(html); // set end ptr
  }
  
  /* Cut off after paragraph, when larger than MAX_PAGE_SIZ bytes */
  /****************************************************************/
#define MAX_PARAGRAPH 30
  else if (strlen(html) > MAX_PAGE_SIZ){
    lt = html;
    for (i=0; i<=MAX_PARAGRAPH; i++){
      lt = strstr(lt,"</p>");
      if (!lt)
        break;
      if (i==MAX_PARAGRAPH){
        strcpy((lt+4),"</body></html>"); // set EOS after </p></body></html>
        eof = lt + strlen(lt); // set end ptr
        break;
      }
      lt += 4;
    }
  ...
  for (lt=html; lt<eof; lt++){
    if (*lt == 0)
      break;
    if (*lt > 0x7e){
      if (*lt = 0xc3){
        switch (*(lt+1)){
          case 0x84: *lt='A'; *(lt+1)='e'; break; // Ä
          case 0x96: *lt='O'; *(lt+1)='e'; break; // Ö
          case 0x9c: *lt='U'; *(lt+1)='e'; break; // Ü
          case 0xa4: *lt='a'; *(lt+1)='e'; break; // ä
          case 0xb6: *lt='o'; *(lt+1)='e'; break; // ö
          case 0xbc: *lt='u'; *(lt+1)='e'; break; // ü
          default:   *lt='*'; break;
        }
      }
      else
  ...
  /* printf("HTML_______________________________________\n%s"
 ...
 set_lt:
	...
	lt = html;
	while (html < eof) {
		unsigned char *name, *attr, *end, *tmp_chr;
		int namelen, nb_loop, curs;
		...
		for (ei = elements; ei->name; ei++) {
          ...
          (strlen(ei->name) != (size_t)namelen || casecmp(ei->name, name, namelen)))
			...
			if (!inv) {
				...
				/*************** Modifications by Martin Pothier - Hach Lange ************/
				// We make the list of the tags of the page
				if (ei->name && ((a = get_attr_val(attr, "src")))) {
					nb_chr = nb_char_tag(end);
					elem = 9;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "href")))) {
          if (*end=='<' || (*end=='\n' && *(end+1)=='<')){
            nb_chr=0;
          }
          else{
            nb_chr = nb_char_tag(end);
				...
				if (ei->name && ((a = get_attr_val(attr, "return")))) {
					nb_chr = nb_char_tag(end);
					elem = 11;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "prev")))) {
					nb_chr = nb_char_tag(end);
					elem = 13;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "home")))) {
					nb_chr = nb_char_tag(end);
					elem = 14;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "form")))) {
					nb_chr = nb_char_tag(end);
					elem = 8;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "height")))) {
					nb_chr = nb_char_tag(end);
					elem = 2;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "xpos")))) {
					nb_chr = nb_char_tag(end);
					elem = 3;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "ypos")))) {
					nb_chr = nb_char_tag(end);
					elem = 4;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "b")))) {
					nb_chr = nb_char_tag(end);
					elem = 5;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "color")))) {
					nb_chr = nb_char_tag(end);
					elem = 6;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "bgcolor")))) {
					nb_chr = nb_char_tag(end);
					elem = 7;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "font")))) {
					nb_chr = nb_char_tag(end);
					elem = 12;
					special(f, SP_TAG, a);
					mem_free(a);
				}
				if (ei->name && ((a = get_attr_val(attr, "width")))) {
					nb_chr = nb_char_tag(end);
					elem = 1;
					special(f, SP_TAG, a);
					mem_free(a);

...
extern struct f_data *fdat;

int hard_write(int fd, unsigned char *p, int l)
{
  // MS: make sure that nothing gets written to console
  return 0;
...
void delete_window(struct window *win)
{
  ...
  del_from_list(win);
  if (win->data) mem_free(win->data), win->data=NULL;
...
void destroy_terminal(struct terminal *term)
{
  ...
  del_from_list(term);
  if (term->fdin >= 0) close(term->fdin);
  if (term->fdout != 1) {
...
void set_color(struct terminal *t, int x, int y, unsigned c)
{
	...
	if (x >= 0 && x < t->x && y >= 0 && y < t->y) {
		t->screen[x + t->x * y] = (t->screen[x + t->x * y] & 0x80ff) | (c & ~0x80ff);
	}
}
...
{
  // printf("Links : set_line\n");
	fdat = (struct f_data *) f_data;
}

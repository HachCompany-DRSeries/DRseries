			      ...
			      struct v4l2_format *f)
{
	...
	if (!xlate){
          ERR("EINVAL: icd=%p, ici=%p, pixelformat=%d",icd,ici,pix->pixelformat);
          return -EINVAL;
        }
...
static struct file *keep_open_file = 0;
...
static int soc_camera_open(struct file *file)
{
	...
	int ret;
        ...
        /* HL workaround 26.7.2017:
           WLAN connection (e.g. with LM005) leads to crash when going to
           standby and waking up again, because soc_camera isn't closed.
           >>> */
        if (keep_open_file)
          soc_camera_close(keep_open_file);
        keep_open_file = file;
        /* <<< HL workaround */
	/*

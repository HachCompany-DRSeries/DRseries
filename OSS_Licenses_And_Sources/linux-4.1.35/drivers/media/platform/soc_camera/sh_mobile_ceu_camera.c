...
#define ERR(fmt,...) printk("ERR: %s[%d]: "fmt"\n",__FUNCTION__,__LINE__,##__VA_ARGS__)
			...
			unsigned int sizes[], void *alloc_ctxs[])
{
	...
	if (fmt) {
		...
		int ret;
                ...
                if (!xlate){
                  ERR("EINVAL: icd=%p, ici=%p, pcdev=%p, pixelformat=%d",icd,ici,pcdev,fmt->fmt.pix.pixelformat);
                  return -EINVAL;
                }
...
static void sh_mobile_ceu_stop_streaming(struct vb2_queue *q)
{
	...
	list_for_each_safe(buf_head, tmp, &pcdev->capture) {
		vb2_buffer_done(&list_entry(buf_head, struct sh_mobile_ceu_buffer, queue)->vb, VB2_BUF_STATE_ERROR);
...
static int sh_mobile_ceu_set_bus_param(struct soc_camera_device *icd)
{
	...
	if (!ret) {
		common_flags = soc_mbus_config_compatible(&cfg,
							  common_flags);
		if (!common_flags){
                  ERR("EINVAL: icd=%p, ici=%p, pcdev=%p, type=%x, flags=%x",icd,ici,pcdev,cfg.type,cfg.flags);
                  return -EINVAL;
                }
...
static int sh_mobile_ceu_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_SHARPNESS:
        ...
        ERR("EINVAL: icd=%p, ici=%p, pcdev=%p, id=%d",icd,ici,pcdev,ctrl->id);
	return -EINVAL;
}
				     ...
				     struct soc_camera_format_xlate *xlate)
{
	...
	if (!icd->host_priv) {
		...
		cam = kzalloc(sizeof(*cam), GFP_ATOMIC);
		if (!cam)
			return -ENOMEM;
				  ...
				  const struct v4l2_crop *a)
{
	...
	if (mf.width > pcdev->max_width || mf.height > pcdev->max_height){
          ERR("EINVAL: icd=%p, ici=%p, pcdev=%p, width=%d, height=%d",icd,ici,pcdev,mf.width,mf.height);
          return -EINVAL;
        }
				 ...
				 struct v4l2_format *f)
{
	...
	if (!xlate) {
		dev_warn(dev, "Format %x not found\n", pixfmt);
                ERR("EINVAL: icd=%p, ici=%p, pcdev=%p, pixfmt=%x",icd,ici,pcdev,pixfmt);
		return -EINVAL;
	}
	...
	if (mf.code != xlate->code){
          ERR("EINVAL: icd=%p, ici=%p, pcdev=%p, code=%ld",icd,ici,pcdev,xlate->code);
          return -EINVAL;
        }
	...
	if (icd->user_width != out_width || icd->user_height != out_height) {
		...
		if (!ret && (out_width != f.fmt.pix.width ||
			     out_height != f.fmt.pix.height)){
                  ERR("EINVAL: icd=%p, ici=%p, pcdev=%p, width=%d, height=%d",icd,ici,pcdev,out_width,out_height);
                  ret = -EINVAL;
                }
		if (!ret) {
			icd->user_width		= out_width & ~3;

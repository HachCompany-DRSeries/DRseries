 ...
 */
#define V4L2_CID_SINGLESHOT		(V4L2_CID_USER_BASE | 0x1002)
#define V4L2_CID_RW_REGISTER		(V4L2_CID_USER_BASE | 0x1003)
#define V4L2_CID_REGISTER_ADR		(V4L2_CID_USER_BASE | 0x1004)
...
*/
	case V4L2_CID_RW_REGISTER:
		ctrl->val = reg_read(client, register_address);
/*
        pr_err( "g_ctrl1 %x\n", register_address);
        pr_err( "g_ctrl2 %x\n", ctrl->val);
*/
...
static int mt9v022_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_HFLIP:
		...
		return 0;
	case V4L2_CID_AUTOGAIN:
//printk("++++ V4L2_CID_AUTOGAIN: val = %d\n", (int)ctrl->val);
		if (ctrl->val)
		...
		return 0;
	case V4L2_CID_EXPOSURE_AUTO:
//printk("++++ V4L2_CID_EXPOSURE_AUTO: val = %d\n", (int)ctrl->val);
		if (ctrl->val)
			 data = reg_set(client, MT9V022_AEC_AGC_ENABLE, 0x1); /* enable auto exposure */
		else
			  data = reg_clear(client, MT9V022_AEC_AGC_ENABLE, 0x1); /* disable auto exposure */
		if (data < 0)
			  return -EIO;
		return 0;
	case V4L2_CID_GAIN:
	{
		struct v4l2_ctrl *gain = mt9v022->gain;
		unsigned long range;
		unsigned long gain_val;
		
		if ( (ctrl->val > gain->maximum) || (ctrl->val < gain->minimum))
			return -EINVAL;
		else {
			/* mt9v022 has minimum == default */
			range = gain->maximum - gain->minimum;
			/* Valid values 16 to 64, 32 to 64 must be even. */
			gain_val = ((ctrl->val - (s32)gain->minimum) *
					      48 + range / 2) / range + 16;
//printk("++++ V4L2_CID_GAIN: max = %d, min = %d, val = %d, g_val = %d\n", (int)gain->maximum, (int)gain->minimum, (int)ctrl->val, (int)gain->val);
	...
	case V4L2_CID_EXPOSURE:
	{
		...
		if ((ctrl->val > exp->maximum) || (ctrl->val < exp->minimum) ) {
			return -EINVAL;
		} else {
			range = exp->maximum - exp->minimum;
			shutter = ((ctrl->val - (s32)exp->minimum) *
					479 + range / 2) / range + 1;

//printk("++++ V4L2_CID_EXPOSURE: max = %d, min = %d, val = %d, e_val = %d\n", (int)exp->maximum, (int)exp->minimum, (int)ctrl->val, (int)exp->val);
//printk("++++ V4L2_CID_EXPOSURE: shutter = %d, range = %d\n", (int)shutter, (int)range);
			/*
 ...
 */
	case V4L2_CID_RW_REGISTER:
		data = reg_write(client, register_address, ctrl->val);
/*
        pr_err( "s_ctrl1 %x\n", register_address);
        pr_err( "s_ctrl2 %x\n", ctrl->val);
*/
...
*/
	...
	case V4L2_CID_SINGLESHOT:
	{
//printk("++++ V4L2_CID_SINGLESHOT: val = %d\n", (int)ctrl->val);
		if (ctrl->val)
		{
			 ...
			 const struct i2c_device_id *did)
{
	...
	v4l2_ctrl_handler_init(&mt9v022->hdl, 20);
	...
	for (i = 0; i < ARRAY_SIZE(mt9v022_additional_ctrls); ++i)
		v4l2_ctrl_new_custom(&mt9v022->hdl, &mt9v022_additional_ctrls[i], NULL);
	...
	v4l2_ctrl_auto_cluster(2, &mt9v022->autoexposure, 0, true);
	v4l2_ctrl_auto_cluster(2, &mt9v022->autogain, 0, true);
	v4l2_ctrl_cluster(2, &mt9v022->gain);
	v4l2_ctrl_cluster(2, &mt9v022->exposure);

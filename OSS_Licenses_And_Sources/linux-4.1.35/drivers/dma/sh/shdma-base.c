	...
	unsigned long flags)
{
	 ...
	 * space.
	 */
	sgl = kcalloc(sg_len, sizeof(*sgl), GFP_ATOMIC);
	if (!sgl)
		return NULL;
			   ...
			   struct shdma_chan *schan, int id)
{
	...
	pm_runtime_get_noresume(schan->dev);
}
		    ...
		    int chan_num)
{
	...
	sdev->schan = kcalloc(chan_num, sizeof(*sdev->schan), GFP_ATOMIC);
	if (!sdev->schan)
		return -ENOMEM;

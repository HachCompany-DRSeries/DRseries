...
static void sci_port_enable(struct sci_port *sci_port)
{
	...
	// printk("%s: RATE=%ld\n", __FUNCTION__, sci_port->iclk->rate);
		...
		if (port->type == PORT_SCI) {
			...
			for (i = 0; i < count; i++) {
				...
				if (status & SCxSR_FER(port)) {
					flag = TTY_FRAME;
					port->icount.frame++;
					dev_notice(port->dev, "frame error (1)\n");
...
static int sci_handle_errors(struct uart_port *port)
{
	...
	if (status & SCxSR_FER(port)) {
		if (sci_rxd_in(port) == 0) {
			...
			dev_notice(port->dev, "frame error (2)\n");
		}
	}
			...
			unsigned long phase, void *p)
{
	...
	if (phase == CPUFREQ_POSTCHANGE) {
		...
		spin_lock_irqsave(&port->lock, flags);
		// printk("%s: RATE=%ld", __FUNCTION__, sci_port->iclk->rate);
			   ...
			   struct plat_sci_port *p, bool early)
{
	...
	case PORT_SCIFA:
		port->fifosize = 64;
		sci_port->overrun_bit = 9;
		sampling_rate = 8; // MS: used to be 16, but RFID ttySC5 was too fast
		break;
	case PORT_SCIF:
		port->fifosize = 16;
